...
class L2Adversary(object):
    ...
    def __call__(self, model, inputs, targets, to_numpy=True):
        ...
        num_classes = model(Variable(inputs[0][None, :], requires_grad=False).cuda()).size(1)  # type: int
        ...
        inputs_tanh_var = Variable(inputs_tanh, requires_grad=False).cuda()
        ...
        targets_oh.scatter_(1, targets.unsqueeze(1), 1) # 3rd arg was 1.0. 1 fixed it
        targets_oh_var = Variable(targets_oh, requires_grad=False).cuda()
        ...
        pert_tanh_var = Variable(pert_tanh, requires_grad=True).cuda()
        ...
        for sstep in range(self.binary_search_steps):
            ...
            scale_consts_var = Variable(scale_consts, requires_grad=False).cuda()
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,
        ...
        advxs_var = self._from_tanh_space(inputs_tanh_var + pert_tanh_var).cuda()  # type: Variable
        # the perturbed activation before softmax
        pert_outputs_var = model(advxs_var).cuda()  # type: Variable
        # the original inputs
        inputs_var = self._from_tanh_space(inputs_tanh_var).cuda()  # type: Variable
        ...
        batch_loss_var = torch.sum(perts_norm_var + c_var * f_var).cuda()  # type: Variable
    ...
    def _compensate_confidence(self, outputs, targets):
        ...
        if self.targeted:
            ...
            # print("outputs_comp", outputs_comp)
            # print("rng", type(rng))
            # print("targets", type(targets))
            outputs_comp[rng[0], int(targets[0])] -= self.confidence
        else:
            # for each image $i$:
